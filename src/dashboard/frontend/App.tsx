...
type DeviceConfig = { host: string; port: number; intervalMs: number; reconnectDelayMs: number; routeFile?: string; drivingStyle: string; seed: number; deviceProfile: string; packetCount?: number };
...
type LogEvent = { id: string; imei?: string; severity: Severity; type: string; message: string; timestampMs: number };

const emptyConfig: DeviceConfig = { host: "127.0.0.1", port: 5027, intervalMs: 1000, reconnectDelayMs: 3000, routeFile: "", drivingStyle: "normal", seed: 1, deviceProfile: "default-codec8e", packetCount: undefined };
const emptyForm = { imei: "", label: "", enabled: true, config: { ...emptyConfig } };
...
export function App(): ReactElement {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [deviceResponse, statusResponse, overviewResponse, logResponse] = await Promise.all([
  ...
  const changeConfig = (name: keyof DeviceConfig, value: string) => setForm((current) => ({ ...current, config: { ...current.config, [name]: ["port", "intervalMs", "reconnectDelayMs", "seed", "packetCount"].includes(name) ? numberValue(value) : value } }));
      ...
      <div style={styles.container}>
        ...
        <div style={styles.columns}>
          <section style={styles.panel}><h2>Device setup</h2><form onSubmit={submitDevice}>
            <div style={styles.formGrid}><label>IMEI<input value={form.imei} disabled={Boolean(selectedDevice) || formActive} onChange={(event) => setForm({ ...form, imei: event.target.value })} placeholder="15 digits" /></label><label>Display name<input value={form.label} disabled={formActive} onChange={(event) => setForm({ ...form, label: event.target.value })} /></label></div>
            ...
            <fieldset disabled={formActive}><legend>Simulator settings</legend><div style={styles.formGrid}>{([ ["host", "Parser host", "text"], ["port", "Parser port", "number"], ["intervalMs", "Interval (ms)", "number"], ["reconnectDelayMs", "Reconnect delay (ms)", "number"], ["routeFile", "Route file", "text"], ["drivingStyle", "Driving style", "text"], ["seed", "Seed", "number"], ["deviceProfile", "Device profile", "text"], ["packetCount", "Packet limit", "number"]] as const).map(([name, label, type]) => <label key={name}>{label}<input type={type} value={form.config[name] ?? ""} onChange={(event) => changeConfig(name, event.target.value)} /></label>)}</div></fieldset>
